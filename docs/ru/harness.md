...
Это тот же механизм (path-repository + `symlink: true`), которым пакет устанавливается в README у [реального стороннего потребителя](../../README.ru.md#установка) — разница только в том, что harness живёт внутри самого репозитория пакета и предназначен для разработчика, а не для конечного пользователя.
...
- [Установка пакета в стороннем приложении](../../README.ru.md#установка) — тот же механизм path-repository, но для реального потребителя пакета
- [Консольные команды](../../README.ru.md#консольные-команды) — команды `build` и `yazar:install`, которые используются внутри harness
- [Конфигурация](configuration.md) — что публикует `yazar:install` и за что отвечает каждая опция `config/yazar.php`
